Shader "Custom/ComicDots"
{
	Properties
	{
		...
		_Frequency("Frecuency", Range(0,1)) = 0
	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 frag(v2f i) : SV_Target
			{
				...
				float3 shadow = floor(max(dot(L, N), 0) * _Levels) * scaleFactor * attenuation;
				...
				float radius = sqrt(1.0 - shadow.r); // Use green channel to modulate dot size
				
				// Add reflections from cube map and add the shadow dots pattern
				float4 reflectionColor = lerp(HSB2RGB(baseColor), cubemapColor, (1 + dot(N, E)) * _Reflectivity);
				float4 finalColor = lerp(_ShadowColor, reflectionColor, lerp(aastep(pow(radius, _Radius), dist), 1, _SoftShadows * shadow.r));
